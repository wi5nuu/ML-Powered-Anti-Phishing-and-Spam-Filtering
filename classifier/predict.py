...
from classifier.features import (
    EmailParser, FeatureExtractor, STRUCTURED_FEATURES, get_protected_domains
)
...
class ModelState:
    ...
    tfidf = None
    scaler = None
    contribution_model = None  # native XGBoost SHAP contribution source
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        # XGBoost exposes exact TreeSHAP contributions natively. This avoids
        # the shap.TreeExplainer parser incompatibility with XGBoost 3.x model
        # configs (base_score is serialized as an array in current releases).
        state.contribution_model = state.model.get_booster()
    ...
    try:
        ...
        _tld.TLDExtract()(f"http://{next(iter(get_protected_domains()))}")
        from langdetect import detect as _detect
        _detect("test")
        from Sastrawi.Stemmer.StemmerFactory import StemmerFactory
        StemmerFactory().create_stemmer().stem("testing")
        import numpy as np
        if state.contribution_model is not None:
            _dummy = np.zeros((1, len(state.all_feature_names)), dtype=np.float32)
            contributions = state.contribution_model.predict(
                xgb.DMatrix(_dummy), pred_contribs=True, validate_features=False
            )
            if contributions.shape[1] != len(state.all_feature_names) + 1:
...
def _predict_supervised_internal(raw_email: str, email_id: str) -> PredictResponse:
    ...
    if state.contribution_model is not None:
        try:
            # The final column is the model bias term; feature contributions
            # occupy the same order as the model input matrix.
            shap_vals = state.contribution_model.predict(
                xgb.DMatrix(X), pred_contribs=True, validate_features=False
            )[0, :-1]
        except Exception as e:
            logger.warning("XGBoost SHAP contributions failed: %s", e)
            shap_vals = None
    else:
        shap_vals = None
    ...
    if prob > 0.30:
        ...
        # Authentication-Results in an inbound MIME message are supplied by
        # the sender and are not trustworthy evidence. SPF/DKIM explanations
        # are added later only from receiver-verified results persisted by the
        # worker; absence of a verified PASS must never be described as FAIL.
