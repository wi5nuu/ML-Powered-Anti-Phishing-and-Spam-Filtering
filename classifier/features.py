...
class FeatureExtractor:
    ...
    def extract(self, parsed: ParsedEmail) -> EmailFeatures:
        ...
        protected_domains = get_protected_domains()
        lev_distances = [
            Levenshtein.distance(d, protected)
        ...
        if lev_distances:
            ...
            features.has_lookalike_domain = (
                features.min_levenshtein_to_protected <= 3
                and any(d not in protected_domains for d in url_domains)
            )
        else:
        ...
        trusted_domains = {
            *get_protected_domains(),
            "bca.co.id", "mandiri.co.id", "bni.co.id",
