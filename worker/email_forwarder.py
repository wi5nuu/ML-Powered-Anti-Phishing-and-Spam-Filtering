...
CLASSIFICATION_HEADER = "X-CogniMail-Classification"
...
def build_warning_xai_header(fused_score: float, payload: dict) -> str:
    """Build an auditable WARN header using only real pipeline outputs."""
    def clean(value) -> str:
        return re.sub(r"[\r\n]+", " ", str(value or "")).strip()

    parts = [
        "CogniMail WARN",
        f"Fused={float(fused_score):.4f}",
        f"ML={float(payload.get('ml_probability') or 0.0):.4f}",
        f"SpamAssassin={float(payload.get('sa_score') or 0.0):.2f}",
        f"Anomaly={float(payload.get('anomaly_score') or 0.0):.4f}",
    ]
    shap_features = payload.get("shap_features") or []
    top_shap = []
    for feature in shap_features[:3]:
        if not isinstance(feature, dict) or not feature.get("name"):
            continue
        try:
            value = float(feature.get("shap", 0.0))
        except (TypeError, ValueError):
            continue
        top_shap.append(f"{clean(feature['name'])}:{value:+.4f}")
    if top_shap:
        parts.append(f"TopSHAP={','.join(top_shap)}")
    routing_reason = clean(payload.get("routing_reason"))
    if routing_reason:
        parts.append(f"Reason={routing_reason}")
    return "; ".join(parts)
...
def _prepare_forward_message(
    ...
    if fusion_label == "WARN":
        message[CLASSIFICATION_HEADER] = "WARN"
        message[SPAM_HEADER] = xai_reason or build_warning_xai_header(fused_score, {})
...
async def forward_email(raw_email: str, fusion_label: str, fused_score: float,
    ...
    try:
        ...
        email_to_send = _prepare_forward_message(
            ...
            fusion_label,
            fused_score,
            build_warning_xai_header(fused_score, payload) if fusion_label == "WARN" else "",
        )
        email_to_send = sign_outbound_message(email_to_send, envelope_from)
