...
from database.models import AdminMailbox, SystemSetting
...
SMTP_TLS_DISABLED = os.getenv("SMTP_TLS_DISABLED", "false").lower() in {"1", "true", "yes", "on"}
RUNTIME_ENV = os.getenv("ENV", "production").strip().lower()
...
async def domain_is_configured(domain: str) -> bool:
    """Check startup domains plus the domain persisted by Super Admin."""
    normalized = str(domain or "").strip().lower().rstrip(".")
    if normalized in ACCEPTED_MAIL_DOMAINS:
        return True
    try:
        _, DbSession = _get_db_session()
        async with DbSession() as db:
            result = await db.execute(
                select(SystemSetting.value).where(
                    ...
                    func.lower(SystemSetting.value) == normalized,
                )
            )
            return result.scalar_one_or_none() is not None
    except Exception as exc:
        logger.error("Organization domain lookup failed for %s: %s", normalized, exc)
        return False
...
class EmailReceiverHandler:
    async def handle_RCPT(self, server: "SMTP", session, envelope, address, rcpt_options):
        ...
        if not normalized or not domain:
            logger.warning("Rejected invalid SMTP recipient=%s peer=%s", normalized, session.peer)
            return "550 5.1.3 Bad recipient address syntax"
        if not await domain_is_configured(domain):
            logger.warning("Rejected non-local SMTP recipient=%s peer=%s", normalized, session.peer)
            return "550 5.7.1 Relay access denied"
        try:
            # The active mailbox table is the authoritative recipient/domain
            # configuration. This makes a domain saved by Super Admin effective
            # immediately while still preventing relay to unknown addresses.
    ...
    async def handle_DATA(self, server: "SMTP", session, envelope):
        ...
        raw_hash = hashlib.sha256(raw_email.encode()).hexdigest()
        # Every accepted SMTP delivery is a distinct event. Keep the full hash
        # separately for forensic deduplication instead of using it as the
        # unique database primary identifier.
        email_id = f"smtp_{uuid.uuid4().hex}"
...
def _build_tls_context() -> ssl.SSLContext | None:
    ...
    if SMTP_TLS_DISABLED:
        if RUNTIME_ENV == "production" or SMTP_REQUIRE_TLS:
            raise RuntimeError("SMTP TLS cannot be disabled in production or when SMTP_REQUIRE_TLS=true")
        return None
    if not SMTP_TLS_CERT or not SMTP_TLS_KEY:
        if SMTP_REQUIRE_TLS:
            raise RuntimeError("SMTP_REQUIRE_TLS=true but SMTP TLS certificate/key is not configured")
        return None
    try:
        ...
        if RUNTIME_ENV == "production" or SMTP_REQUIRE_TLS:
            raise RuntimeError(f"Failed to load SMTP TLS certificate/key: {exc}") from exc
        logger.warning("Failed to load TLS cert/key in local mode: %s — STARTTLS disabled", exc)
        return None
