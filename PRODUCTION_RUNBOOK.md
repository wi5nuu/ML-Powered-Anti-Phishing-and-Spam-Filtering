...
- set `DASHBOARD_DOMAIN` ke hostname dashboard publik, misalnya
  `cognimail.zenime.my.id`
- set `VITE_MAIL_DOMAIN` dan `ACCEPTED_MAIL_DOMAINS` ke domain alamat mailbox,
  misalnya `zenime.my.id`. Domain mailbox tidak harus sama dengan hostname
  dashboard.
- set `SMTP_DOMAIN`, `OUTBOUND_HELO_HOSTNAME`, `SMTP_TLS_CERT`, dan
  `SMTP_TLS_KEY` ke hostname/certificate yang benar-benar tersedia di VPS.
...
- pilih mode SMTP outbound untuk kirim, balas, dan forward mailbox.
  ...
  OUTBOUND_HELO_HOSTNAME=${DASHBOARD_DOMAIN}
  ...
  sender. Contoh: email yang masuk ke `bantuan@${VITE_MAIL_DOMAIN}` dan diteruskan ke
  Gmail tetap dikirim oleh `bantuan@${VITE_MAIL_DOMAIN}`, sedangkan mailbox
  `sales@${VITE_MAIL_DOMAIN}` memakai `sales@${VITE_MAIL_DOMAIN}`. Tujuan forward disimpan
  ...
  Untuk IP publik VPS Anda, ubah PTR/rDNS melalui panel provider menjadi
  `${DASHBOARD_DOMAIN}`. Pastikan record A hostname tersebut kembali ke IP
  yang sama. Tanpa forward-confirmed reverse DNS, server seperti Gmail
  ...
  FORWARDER_SMTP_USER=bantuan@${VITE_MAIL_DOMAIN}
  FORWARDER_SMTP_PASS=app-password-atau-password-smtp
  FORWARDER_FROM=bantuan@${VITE_MAIL_DOMAIN}
  ```
...
- simpan hanya kredensial break-glass `SUPERADMIN_USERNAME` dan
  `SUPERADMIN_PASSWORD` di `.env`. Akun admin dan user dikelola di PostgreSQL
  melalui dashboard. Gunakan nilai acak yang kuat untuk
  `SUPERADMIN_PASSWORD`, `GF_SECURITY_ADMIN_PASSWORD`, dan
