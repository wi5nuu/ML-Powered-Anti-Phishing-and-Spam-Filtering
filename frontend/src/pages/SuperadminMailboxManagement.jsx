...
import {
  ...
  AtSign, User, Users, HardDrive, Shield, ShieldAlert,
  ChevronDown, ChevronUp, ExternalLink, AlertCircle,
  MoreVertical, Edit3, Trash2, ToggleLeft, ToggleRight,
  KeyRound, Eye, EyeOff, Copy, Forward
} from 'lucide-react'
import styles from './SuperadminMailboxManagement.module.css'
...
export default function SuperadminMailboxManagement() {
  ...
  const [editForm, setEditForm] = useState({ email: '', domain: '', sender_name: '', assigned_to: '' })
  ...
  const openEdit = (mb) => {
    setEditTarget(mb)
    setEditForm({ email: mb.email, domain: mb.domain, sender_name: mb.sender_name, assigned_to: mb.assigned_to || '' })
  }

  const handleEdit = async () => {
    if (!editTarget) return
    try {
      await api.put(`/admin/mailboxes/${editTarget.id}`, editForm)
  ...
  const openForwarder = (mb) => {
    setForwardTarget(mb)
    setForwardTo(mb.forward_to || '')
    setForwardKeep(mb.forward_keep_copy !== false)
  }

  const handleForwarder = async () => {
    if (!forwardTarget) return
    try {
      await api.put(`/admin/mailboxes/${forwardTarget.id}/forwarder`, {
        target: forwardTo,
        enabled: !!forwardTo,
        keep_copy: forwardKeep,
      })
      showMsg('Forwarder berhasil disimpan')
      setForwardTarget(null)
      fetchMailboxes()
    } catch (err) {
      showMsg(err.response?.data?.detail || 'Gagal menyimpan forwarder', 'error')
    }
  }
  ...
  return (
    <div className={styles.page}>
          ...
          <tbody>
            {sorted.length === 0 ? (
              ...
              <tr key={mb.id}>
                ...
                <td>
                  {mb.assigned_to ? (
                    <span className={`${styles.assignBadge} ${userMap[mb.assigned_to]?.role === 'admin' ? styles.assignAdmin : styles.assignUser}`}>
                      {userMap[mb.assigned_to]?.role === 'admin' ? <Shield size={12} /> : <User size={12} />}
                      {mb.assigned_to}
                    </span>
                  ) : (
                ...
                <td>
                  <div className={styles.actionGroup}>
                    ...
                    <button className={styles.actionBtn} onClick={() => openForwarder(mb)} title="Forwarder">
                      <Forward size={15} />
                    </button>
                    <button
        ...
        <div className={styles.overlay} onClick={() => setShowAdd(false)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              ...
              <div className={styles.fieldRow}>
                <div className={styles.fieldLeft}>
                  <label className={styles.fieldLabel}><Users size={14} /> Assign To <span style={{ color: '#DC2626' }}>*</span></label>
                  <span className={styles.fieldHint}>Admin atau User yang akan mengelola email ini</span>
                </div>
                <div className={styles.fieldRight}>
                  ...
                  {users.filter(u => u.role === 'admin').length === 0 && (
                    <span style={{ fontSize: '0.75rem', color: '#DC2626', marginTop: 4, display: 'block' }}>
                      Belum ada admin. Buat admin di menu Users terlebih dahulu.
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            ...
            <div className={styles.modalBody}>
              <div className={styles.fieldRow}>
                ...
                <div className={styles.fieldRight}>
                  <input className={styles.input} value={editForm.email} onChange={(e) => setEditForm(f => ({ ...f, email: e.target.value }))} />
                </div>
              </div>
              <div className={styles.fieldRow}>
                <div className={styles.fieldLeft}>
                  <label className={styles.fieldLabel}><AtSign size={14} /> Domain</label>
                </div>
                <div className={styles.fieldRight}>
                  <input className={styles.input} value={editForm.domain} onChange={(e) => setEditForm(f => ({ ...f, domain: e.target.value }))} />
                </div>
              </div>
              <div className={styles.fieldRow}>
                <div className={styles.fieldLeft}>
      ...
      {forwardTarget && (
        <div className={styles.overlay} onClick={() => setForwardTarget(null)}>
          <div className={styles.modal} onClick={(e) => e.stopPropagation()}>
            <div className={styles.modalHeader}>
              <h3><Forward size={18} /> Forwarder</h3>
              <button className={styles.closeBtn} onClick={() => setForwardTarget(null)}><X size={20} /></button>
            </div>
            <div className={styles.modalBody}>
              <p className={styles.pwdTargetLabel}>{forwardTarget.email}</p>
              <div className={styles.fieldRow}>
                <div className={styles.fieldLeft}>
                  <label className={styles.fieldLabel}><Mail size={14} /> Forward To</label>
                </div>
                <div className={styles.fieldRight}>
                  <input className={styles.input} placeholder="forward@example.com" value={forwardTo} onChange={(e) => setForwardTo(e.target.value)} autoFocus />
                </div>
              </div>
              <label className={styles.checkRow}>
                <input type="checkbox" checked={forwardKeep} onChange={(e) => setForwardKeep(e.target.checked)} />
                <span>Simpan salinan email yang diteruskan</span>
              </label>
            </div>
            <div className={styles.modalFooter}>
              <button className={styles.cancelBtn} onClick={() => setForwardTarget(null)}>Tutup</button>
              <button className={styles.saveBtn} onClick={handleForwarder}>
                <Check size={16} /> Simpan
              </button>
            </div>
          </div>
