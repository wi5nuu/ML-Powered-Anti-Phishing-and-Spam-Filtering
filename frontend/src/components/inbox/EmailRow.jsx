...
export default function EmailRow({
  ...
  const securityLabel = ['clean', 'spam', 'phishing', 'malware', 'quarantine'].includes(verdict) ? verdict : null
    ...
    <>
      ...
      <div className={styles.body}>
        <span className={styles.subject}>
          {securityLabel && (
            <span className={`${styles.securityBadge} ${styles[`securityBadge_${securityLabel}`]}`}>
              {securityLabel === 'clean' ? 'Safe' : securityLabel.charAt(0).toUpperCase() + securityLabel.slice(1)}
            </span>
          )}
