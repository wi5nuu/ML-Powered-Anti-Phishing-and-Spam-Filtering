...
def fuse(sa_score: float, ml_probability: float,
    ...
    # A supervised probability this close to 1.0 is itself decisive evidence.
    # Previously it was discarded whenever SA/anomaly did not also cross their
    # hard thresholds, converting confirmed threats into CLEAN messages.
    ml_is_decisive = ml_probability >= ML_DECISIVE_LIMIT
    if hard_evidence and (
        ml_is_decisive
        or not QUARANTINE_EVIDENCE_GATED
        or len(hard_evidence) >= QUARANTINE_MIN_EVIDENCE
    ):
    ...
    if fused < THRESH_CLEAN:
        ...
        if WARN_EVIDENCE_GATED and len(evidence) < WARN_MIN_EVIDENCE:
            # A score in the warning band is uncertainty, not proof that the
            # message is clean. WARN messages are held for administrator review
            # by the worker and are never forwarded to the mailbox.
            label = "QUARANTINE" if ml_probability >= ML_REVIEW_LIMIT else "WARN"
            reason = (
