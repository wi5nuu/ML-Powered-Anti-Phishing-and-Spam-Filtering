...
export default function EmailList({ view = '', activeEmailId = null }) {
  ...
  const category = ['spam', 'phishing'].includes(view)
    ? view
    : canonicalEmailCategory(searchParams.get('category'))
  const labelView = view === 'warn' || category === 'warn' ? 'warn' : ''
  ...
  const apiFilter = labelView || category || folder || filter
  ...
  useEffect(() => {
    setSelected(new Set())
  }, [filter, folder, category, labelView, query, mailbox, mailboxId, page])

  useEffect(() => {
    const pageParam = searchParams.get('page')
    if (!pageParam || (Number.isFinite(rawPage) && rawPage >= 1)) return
    setSearchParams((previous) => {
      const next = new URLSearchParams(previous)
      next.delete('page')
      return next
    }, { replace: true })
  }, [rawPage, searchParams, setSearchParams])
  ...
  const threads = useMemo(() => {
    ...
    const skipGrouping = folder === 'trash' || folder === 'starred' || category || labelView
  ...
  }, [emails, folder, category, labelView, mailboxId])
