...
import { canonicalEmailCategory } from '../../utils/emailCategory'
...
export default function GmailShell({ children }) {
  ...
  const user = me?.user
  const canReviewThreats = ['admin', 'superadmin'].includes(user?.role)
  ...
  const { data: liveProfile } = useProfile(mailboxId)
  ...
  // The authenticated account can be an admin opening a managed mailbox.
  // Never use that admin's avatar as the mailbox avatar. The authenticated
  // avatar is a valid fallback only when the account itself is a mailbox.
  const authenticatedMailboxAvatar = user?.role === 'mailbox' ? user?.avatar_url || '' : ''
  const mailboxAvatarUrl = liveProfile?.avatar_url || activeMailboxSession?.avatar_url || activeMailboxDirectory?.avatar_url || authenticatedMailboxAvatar
  const avatarUrl = mailboxIdentity ? mailboxAvatarUrl : liveProfile?.avatar_url || user?.avatar_url || ''
  ...
  useEffect(() => {
    ...
    const isRestrictedPath = /^\/mail\/[^/]+\/(all|trash)\/?$/.test(location.pathname)
    const requestedFolder = searchParams.get('folder') || ''
    const isRestrictedQuery = ['allmail', 'all', 'trash'].includes(requestedFolder)
    if (!isRestrictedPath && !isRestrictedQuery) return

    // Regular users only work from the clean inbox. Do not leave the removed
    // All Mail view accessible through an old bookmark or browser history.
    navigate(withMailbox('/inbox', mailboxIdentity, mailboxId), { replace: true })
  }, [canReviewThreats, location.pathname, mailboxId, mailboxIdentity, navigate, searchParams, user])
  ...
  }, [user, canReviewThreats, location.pathname, searchParams, navigate, mailboxIdentity, mailboxId])
  ...
  const getSearchContext = () => {
    ...
    if (searchBase === '/inbox') {
      ...
      const category = ['spam', 'phishing', 'warn', 'malware'].includes(mailSection)
  ...
  const isNavActive = (to) => {
    ...
    const logicalCurrentPath = (() => {
      ...
      if (['spam', 'phishing', 'warn', 'malware'].includes(section)) return '/inbox'
    ...
    if (path === '/inbox') {
      ...
      const currentCategory = ['spam', 'phishing', 'warn', 'malware'].includes(mailSection)
        ? canonicalEmailCategory(mailSection)
        : canonicalEmailCategory(currentParams.get('category'))
  ...
  const currentCat = ['spam', 'phishing', 'warn', 'malware'].includes(activeMailSection)
    ? canonicalEmailCategory(activeMailSection)
    : activeSourcePath.startsWith('/inbox')
      ? (canonicalEmailCategory(activeSourceParams.get('category')) || null)
      : null
