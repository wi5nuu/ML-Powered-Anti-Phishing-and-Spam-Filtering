          ...
          {loading ? <div className={styles.empty}>Memuat mailbox...</div> : filtered.length === 0 ? (
            ...
            <table className={styles.table}>
              ...
              <tbody>{filtered.map((mailbox) => (
                <tr key={mailbox.id}>
                  <td><div className={styles.emailCell}><span className={hasUploadedAvatar(mailbox.avatar_url) ? styles.mailboxAvatar : undefined}>{hasUploadedAvatar(mailbox.avatar_url) ? <img src={mailbox.avatar_url} alt={`Foto profil ${mailbox.email}`} /> : (mailbox.email || '?')[0].toUpperCase()}</span><div><strong>{mailbox.email}</strong><small>@{mailbox.domain || domain}</small></div></div></td>
