...
export default function AdminDetectionLogs() {
  ...
  const fetchLogs = (p = 1, q = search, mailbox = mailboxFilter) => {
  ...
  const handleSearch = (e) => { e.preventDefault(); setPage(1); setSearch(searchInput); fetchLogs(1, searchInput, mailboxFilter) }
  const handleMailbox = (value) => { setMailboxFilter(value); setPage(1); fetchLogs(1, search, value) }
  const handleClear  = () => { setSearchInput(''); setSearch(''); setMailboxFilter(''); setPage(1); fetchLogs(1, '', '') }
      ...
      <div className={styles.section}>
        ...
        <div className={styles.filterBar}>
          <form onSubmit={handleSearch} className={styles.searchForm}>
            ...
            />
            {(searchInput || search || mailboxFilter) && (
          ...
          <select className={styles.filterSelect} value={mailboxFilter} onChange={(e) => handleMailbox(e.target.value)}>
            <option value="">{t('logs.allMailboxes')}</option>
        ...
        {loading ? (
          ...
          <>
            <table className={styles.table}>
              ...
              <tbody>
                {logs.map((log) => {
                  const displayCategory = String(log.label || '').toUpperCase() === 'WARN'
                    ? 'warn'
                    : log.category === 'malware' ? 'phishing' : log.category
                  ...
                  return (
                    <tr key={log.email_id || log.id}>
                      ...
                      <td className={styles.mono} style={{ fontSize: '0.75rem', fontWeight: 600 }}>{log.mailbox}</td>
                      <td>
