...
import {
  ...
  Mail, Users, TrendingUp, AlertTriangle, CheckCircle,
  Calendar,
} from 'lucide-react'
...
export default function ThreatReportPage() {
  ...
  const maxCat = Math.max(cc.phishing || 0, cc.spam || 0, cc.warn || 0, 1)
  const totalThreats = (cc.quarantine || 0) + (cc.warn || 0)
  const recipients = data?.top_recipients || []
  const senders = data?.top_senders || []
  const trend = data?.daily_trend || []
  // The API keeps trend data chronological for charts. The report table is a
  // recent-activity view, so show the newest ISO date first without mutating
  // the response array used elsewhere.
  const tableTrend = [...trend].sort((left, right) =>
            ...
            <div className={styles.panel}>
              ...
              <div className={styles.catRows}>
                {[
                  ...
                  { label: t('report.warning', 'Peringatan'), value: cc.warn ?? 0, color: THREAT_COLORS.warn.bar },
                ].map((item) => (
                  ...
                  <tbody>
                    {senders.map((s, i) => (
                      <tr key={i}>
                        ...
                        <td className={styles.tdWarn}>{s.warn}</td>
          ...
          <div className={styles.panel}>
            ...
            <div className={styles.tableWrap}>
              <table className={styles.table}>
                ...
                <tbody>
                  {tableTrend.map((d) => (
                    <tr key={d.date} className={d.quarantine > 0 ? styles.trDanger : ''}>
