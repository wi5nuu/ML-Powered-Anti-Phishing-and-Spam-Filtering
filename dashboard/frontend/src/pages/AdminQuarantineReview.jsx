...
export default function AdminQuarantineReview() {
  ...
  const fetchEmails = (p = 1, q = search, cat = categoryFilter, mailbox = mailboxFilter) => {
  ...
  const handleSearch = (e) => { e.preventDefault(); setPage(1); setSearch(searchInput); fetchEmails(1, searchInput, categoryFilter, mailboxFilter) }
  const handleCat    = (val) => { setCategoryFilter(val); setPage(1); fetchEmails(1, search, val, mailboxFilter) }
  const handleMailbox = (value) => { setMailboxFilter(value); setPage(1); fetchEmails(1, search, categoryFilter, value) }
  const handleClear  = () => { setSearchInput(''); setSearch(''); setCategoryFilter(''); setMailboxFilter(''); setPage(1); fetchEmails(1, '', '', '') }
  ...
  return (
    <div style={{ padding: '0 0 24px' }}>
      ...
      <div className={styles.section}>
        ...
        <div className={styles.filterBar}>
          <form onSubmit={handleSearch} className={styles.searchForm}>
            ...
            />
            {(searchInput || search || categoryFilter || mailboxFilter) && (
              <button type="button" onClick={handleClear} className={styles.clearBtn}>
                <X size={12} />
              </button>
            )}
          </form>
          <select className={styles.filterSelect} value={mailboxFilter} onChange={(e) => handleMailbox(e.target.value)}>
            <option value="">{t('quarantine.allMailboxes')}</option>
          ...
          <select className={styles.filterSelect} value={categoryFilter} onChange={(e) => handleCat(e.target.value)}>
            ...
            <option value="warn">{t('gmail.warn')}</option>
          </select>
        </div>
        ...
        {loading ? (
          ...
          <>
            <table className={styles.table}>
              ...
              <tbody>
                {emails.map((e) => {
                  const displayCategory = String(e.label || '').toUpperCase() === 'WARN'
                    ? 'warn'
                    : e.category === 'malware' ? 'phishing' : e.category
                  const cc = CAT_COLOR[displayCategory] || { bg: '#f1f3f4', text: '#5f6368' }
                  return (
                    <tr key={e.email_id || e.id}>
                      ...
                      <td className={styles.mono} style={{ fontSize: '0.75rem', fontWeight: 600 }}>{e.mailbox}</td>
                      <td>
                        <span className={styles.categoryBadge} style={{ background: cc.bg, color: cc.text }}>
                          {displayCategory === 'warn' ? t('gmail.warn') : displayCategory}
                        </span>
                      </td>
      ...
      <ConfirmDialog
        ...
        title={t('quarantine.confirmDelete', 'Hapus permanen')}
        message={`${deleteTarget?.label || t('quarantine.thisEmail', 'email ini')}? ${t('quarantine.confirmDeleteSuffix', 'Tindakan ini tidak dapat dibatalkan.')}`}
