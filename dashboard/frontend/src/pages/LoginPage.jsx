...
import { ArrowLeft, Eye, EyeOff, Lock, Mail, Shield, ShieldCheck, Zap, Bell } from 'lucide-react'
...
export default function LoginPage() {
  ...
  useEffect(() => {
    ...
    // The primary login page must remain available even when an old mailbox
    // cookie is still valid. A new successful login replaces that old session.
    if (role === 'mailbox') return
      ...
      <div className={styles.right}>
        ...
        {mode === 'login' ? (
            ...
            <div className={styles.field}>
              <div className={styles.labelRow}>
                ...
                <button
                  ...
                  onClick={() => { setMode('forgot'); setError(''); setResetRole('') }}
                >
          ...
          <div className={styles.form}>
            <div className={styles.field}>
              <span className={styles.fieldLabel}>{t('login.resetAccountType')}</span>
              <div className={styles.resetRoleGrid}>
                <button
                  type="button"
                  className={`${styles.resetRoleBtn} ${resetRole === 'user' ? styles.resetRoleActive : ''}`}
                  onClick={() => { setResetRole('user'); setError('') }}
                >
                  <Mail size={17} />
                  <span><strong>{t('login.resetRoleUser')}</strong><small>{t('login.resetRoleUserHint')}</small></span>
                </button>
                <button
                  type="button"
                  className={`${styles.resetRoleBtn} ${resetRole === 'admin' ? styles.resetRoleActive : ''}`}
                  onClick={() => { setResetRole('admin'); setError('') }}
                >
                  <Shield size={17} />
                  <span><strong>{t('login.resetRoleAdmin')}</strong><small>{t('login.resetRoleAdminHint')}</small></span>
                </button>
              </div>
            </div>
            {resetRole && (
              <div className={styles.resetHelpPanel} role="status">
                <ShieldCheck size={19} />
                <div>
                  <strong>{t(resetRole === 'admin' ? 'login.resetHelpAdminTitle' : 'login.resetHelpUserTitle')}</strong>
                  <p>{t(resetRole === 'admin' ? 'login.resetMessageAdmin' : 'login.resetMessageUser')}</p>
                </div>
              </div>
            )}
            <button
              ...
              onClick={() => { setMode('login'); setError(''); setResetRole('') }}
            >
              <ArrowLeft size={16} />
              {t('login.backToLogin')}
            </button>
