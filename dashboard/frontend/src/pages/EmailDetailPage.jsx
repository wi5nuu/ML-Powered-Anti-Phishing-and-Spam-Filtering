...
import { hasUploadedAvatar } from '../utils/avatar'
...
function sanitizeEmailHtml(value = '') {
  ...
  doc.querySelectorAll('*').forEach((node) => {
    Array.from(node.attributes).forEach((attr) => {
      ...
      if (name === 'href' && !/^\s*(?:https?:|mailto:)/i.test(attrValue)) {
        node.removeAttribute(attr.name)
        return
      }
      if (name === 'src' && !/^\s*(?:data:image\/|cid:)/i.test(attrValue)) {
        node.removeAttribute(attr.name)
        node.setAttribute('data-remote-content-blocked', 'true')
      }
    })
...
function splitQuotedThread(value = '') {
  let source = stripRawHeaders(value)
  // Older sent records stored generated HTML as escaped text. Decode that
  // representation before locating gmail_quote so existing conversations are
  // repaired without rewriting historical database rows.
  if (looksLikeEscapedHtml(source)) source = decodeHtmlEntities(source)
...
export default function EmailDetailPage({ overrideEmailId = null }) {
  ...
  const getListFilter = (source) => {
    ...
    if (['spam', 'phishing', 'warn', 'malware'].includes(mailSection)) return canonicalEmailCategory(mailSection)
    ...
    if (category) return canonicalEmailCategory(category)
    return 'all'
  }
  ...
  const { data: mailboxProfile } = useProfile(activeMailboxId)
  ...
  const replySaveQueueRef = useRef(Promise.resolve())
  const autoReadEmailIdRef = useRef('')
  const [securityPanelOpen, setSecurityPanelOpen] = useState(false)
  const [deleteDialogOpen, setDeleteDialogOpen] = useState(false)
  const [openPanels, setOpenPanels] = useState({ actions: true, scores: false, xai: false, metadata: false })
  ...
  useEffect(() => {
    if (email) {
      ...
      const shouldAutoMarkRead = autoReadEmailIdRef.current !== emailId
      if (shouldAutoMarkRead) autoReadEmailIdRef.current = emailId
      if (shouldAutoMarkRead && !currentlyRead && !isDraftEmail) {
  ...
  const canReviewThreats = ['admin', 'superadmin'].includes(role)
  const showSecurityPanel = canReviewThreats
  ...
  const canReportSafe = label === 'quarantine' || label === 'warn'
  const canReportThreat = label === 'clean' || label === 'warn'
  const shap = email.shap_data
  const shapFeatures = Array.isArray(shap?.features)
    ? shap.features.filter((feature) => Number.isFinite(Number(feature?.shap)))
    : []
  const maxShap = shapFeatures.length
    ? Math.max(...shapFeatures.map((feature) => Math.abs(Number(feature.shap))), 0.000001)
    : 1
  ...
  // Admins and superadmins may inspect a managed mailbox. Their personal
  // avatar must not be rendered as the mailbox/sender avatar.
  const authenticatedMailboxAvatar = meData?.user?.role === 'mailbox'
    ? meData?.user?.avatar_url || ''
    : ''
  const mailboxAvatarUrl = mailboxProfile?.avatar_url || authenticatedMailboxAvatar
  const hasMailboxAvatar = hasUploadedAvatar(mailboxAvatarUrl)
      ...
      <div key={message.email_id || `${message.subject}-${index}`} className={styles.threadMessageGroup}>
        ...
        >
          <div className={styles.threadAvatar} style={{ backgroundColor: bg }} title={senderName}>
            {isOwnMessage && hasMailboxAvatar
              ? <img src={mailboxAvatarUrl} alt="" />
              : initial}
          </div>
                ...
                {threadRecipientDetailId === detailId && (
                  <div className={styles.recipientDropdown}>
                    ...
                    <div className={styles.recipientRow}>
                      ...
                      <span className={styles.recipientValue}>{displayEmailCategory(message.category, message.label) || '-'}</span>
                    </div>
                  </div>
                )}
              </div>
  ...
  const persistReplyDraft = async ({
    ...
    const stableDraftId = replyDraftIdRef.current || replyDraftId || createDraftId()
    if (!replyDraftIdRef.current) {
      replyDraftIdRef.current = stableDraftId
      setReplyDraftId(stableDraftId)
    }
    const snapshot = {
      draftId: stableDraftId,
      recipients: Array.from(new Set(validation.recipients || [])).join(', '),
      from: activeMailbox || recipients[0] || meData?.user?.email || meData?.user?.username || '',
      subject: replySubject,
      body: buildReplyDraftBody(),
      parentEmailId: replyTargetMessage?.email_id || email.email_id,
      composeMode: replyMode || 'reply',
      attachments: [...replyAttachments],
    }
    const previousSave = replySaveQueueRef.current
    let releaseQueue
    replySaveQueueRef.current = new Promise((resolve) => { releaseQueue = resolve })
    ...
    try {
      ...
      if (snapshot.attachments.length > 0) {
        ...
        formData.append('to', snapshot.recipients)
        formData.append('from_email', snapshot.from)
        formData.append('subject', snapshot.subject)
        formData.append('body', snapshot.body)
        formData.append('parent_email_id', snapshot.parentEmailId)
        ...
        response = await api.post('/emails/draft', {
          ...
          from_email: snapshot.from,
          subject: snapshot.subject,
          body: snapshot.body,
          parent_email_id: snapshot.parentEmailId,
      ...
      showToast(t('email.failSaveDraft') + (err.response?.data?.detail ? ': ' + err.response.data.detail : ''), 'error')
      return null
    } finally {
  ...
  const handleDiscardReplyDraft = async () => {
    if (replyAutosaveTimerRef.current) clearTimeout(replyAutosaveTimerRef.current)
    await replySaveQueueRef.current.catch(() => {})
  ...
  const handleSendReply = async () => {
    ...
    try {
      ...
      await persistReplyDraft({ silent: true, closeAfter: false, resetAfter: false, requireRecipient: false })
    }
  }
        ...
        <div className={styles.emailPane}>
          ...
          {email.label === 'WARN' && (
            <div className={styles.warningBanner} role="status">
              ...
              <div>
                <strong>{t('email.warningTitle')}</strong>
                <span>{email.warning_xai_header || t('email.warningFallback')}</span>
              </div>
            </div>
          )}
            ...
            <div className={`${styles.securityPane} ${securityPanelOpen ? styles.securityPaneOpen : ''}`}>
              <SecurityPanelWrapper onClose={() => setSecurityPanelOpen(false)} t={t}>
                <SecuritySection id="actions" title={t('panel.actions')} isOpen={openPanels.actions} onToggle={togglePanel}>
                  ...
                  <p className={styles.reviewLead}>{t('feedback.reviewLead')}</p>
                  <div className={styles.actionButtons}>
                    {label === 'quarantine' && (email.category || '').toLowerCase() === 'spam' && (
                      <div className={styles.decisionOption}>
                        <div>
                          <div className={styles.feedbackTitle}>{t('feedback.correctTitle')}</div>
                          <p className={styles.feedbackDescription}>{t('feedback.correctDescription')}</p>
                        </div>
                        <button
                          className={`${styles.btn} ${styles.btnPrimaryDecision}`}
                          ...
                          disabled={spamming}
                        >
                          {spamming ? t('action.processing') : t('feedback.keepAsSpam')}
                        </button>
                      </div>
                    )}
                    {canReportSafe && (
                      <div className={styles.decisionOption}>
                      <div className={styles.feedbackTitle}>{t('feedback.fpTitle')}</div>
                      <p className={styles.feedbackDescription}>{t('feedback.fpDescription')}</p>
                      <button
                        className={`${styles.btn} ${styles.btnSecondaryDecision}`}
                        onClick={() => reportFP({ emailId, notes: '' }, {
                        ...
                        disabled={reporting}
                      >
                        {reporting ? t('action.processing') : t('feedback.fpAction')}
                      </button>
                      <span className={styles.feedbackDestination}>{t('feedback.trainingDestination')}</span>
                      </div>
                    )}
                    {canReportThreat && (
                      <div className={styles.decisionOption}>
                        <div className={styles.feedbackTitle}>{t('feedback.fnTitle')}</div>
                        <p className={styles.feedbackDescription}>{t('feedback.fnDescription')}</p>
                        <select
                        ...
                        <button
                          className={`${styles.btn} ${styles.btnDangerDecision}`}
                          onClick={() => reportFN({ emailId, correctedLabel: fnLabel, notes: '' }, {
                            onSuccess: () => { showToast(t('feedback.fnSuccess'), 'warning'); navigate(backPath) },
                            onError: (err) => showToast(err.response?.data?.detail || t('feedback.fnError'), 'error'),
                          })}
                          disabled={reportingFN}
                        >
                          {reportingFN ? t('action.processing') : t('feedback.fnAction')}
                        </button>
                        <span className={styles.feedbackDestination}>{t('feedback.trainingDestination')}</span>
                      </div>
                    )}
                ...
                {(email.human_reasons?.length > 0 || shapFeatures.length > 0) && (
                  <SecuritySection id="xai" title={t('panel.xai')} isOpen={openPanels.xai} onToggle={togglePanel}>
                    ...
                    {shapFeatures.length > 0 && (
                      <div className={styles.shapList}>
                        <div className={styles.shapLegend}>{t('panel.shapContributions')}</div>
                        {shapFeatures.map((feature, index) => {
                          const value = Number(feature.shap)
                          const direction = feature.direction === 'ham' || value < 0 ? 'ham' : 'spam'
                          return (
                            <div className={styles.shapBar} key={`${feature.name}-${index}`} title={feature.name}>
                              <span className={styles.shapLabel}>{feature.name}</span>
                              <span className={styles.shapTrack}>
                                <span
                                  className={`${styles.shapFill} ${styles[direction]}`}
                                  style={{ width: `${Math.max(3, Math.abs(value) / maxShap * 100)}%` }}
                                />
                              </span>
                              <span className={styles.shapVal}>{value >= 0 ? '+' : ''}{value.toFixed(4)}</span>
                            </div>
                          )
                        })}
                        <div className={styles.shapLegend}>
                          <span>{t('panel.shapSpam')}</span>
                          <span>{t('panel.shapHam')}</span>
                        </div>
                      </div>
                    )}
                  </SecuritySection>
                ...
                <SecuritySection id="metadata" title={t('panel.metadata')} isOpen={openPanels.metadata} onToggle={togglePanel}>
                  <div className={styles.meta}>
                    <div className={styles.metaRow}>
                      ...
                      <span className={styles.metaValue}>{email.label === 'WARN' ? t('gmail.warn') : email.category === 'malware' ? t('gmail.phishing') : (email.category || email.label || '-')}</span>
                    </div>
                    <div className={styles.metaRow}>
